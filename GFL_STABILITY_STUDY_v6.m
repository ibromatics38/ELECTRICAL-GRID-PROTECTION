...
% If your Simulink model expects the full legacy "p" parameter set (v4 style), keep this true.
config.compat.use_legacy_parambase = true;
config.T_end = 5.0;
config.T_stp = 1.3;
...
function p = params_base_compat_fn(config, params, taus, tspll)
% Compatibility parameter base (inspired by v4 param-base style).
% This helps when the Simulink model references legacy variable names.
p = struct();
...
p.f1 = params.fbase;
p.w_g1 = 2*pi*p.f1;
p.Vdcref_n1 = params.Vdc;

% Simulation timing
p.Tsim = params.T_end;
p.T_stp = params.T_stp;
p.Ts = 5e-5;
...
p.w_n2 = 2*pi*p.f_n2;
p.Un_n2 = p.Un1;
p.Vpeak_n2 = p.Un_n2/sqrt(3)*sqrt(2);
p.Inom_n2 = p.Sn_n2/p.Un_n2/sqrt(3);
p.Xn_n2 = p.Un_n2^2/p.Sn_n2;
p.Ln_n2 = p.Xn_n2/(2*pi*p.f_n2);
p.Rc_n2 = params.Rc;
p.Lc_n2 = params.Lc;
p.Cac_n2 = params.Cf;

% Grid side from SCR,RX mapping already computed in params
p.Rac = params.Rg;
p.Lac = params.Lg;
...
% Keep also v6 names for mixed-model compatibility
p.Vdc = params.Vdc;
p.Rg = params.Rg;
p.Lg = params.Lg;
p.Lc = params.Lc;
...
errMsg = '';
try
    if config.compat.use_legacy_parambase
        p = params_base_compat_fn(config, params, taus, tspll);
        fnames = fieldnames(p);
        for kk = 1:numel(fnames)
            assignin('base', fnames{kk}, p.(fnames{kk}));
        end
        assignin('base','p',p);
    else
        xi = 0.707;
